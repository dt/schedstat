// Package tracedb converts Go execution traces to DuckDB databases.
//
// This package is derived from github.com/felixge/sqlprof/db
// Copyright Felix Geisendörfer
// Licensed under the Apache License, Version 2.0
// See https://github.com/felixge/sqlprof/blob/main/LICENSE
//
// Modified to use the updated golang.org/x/exp/trace API where
// Stack.Frames() returns an iterator instead of taking a callback.
package tracedb
...
// DB wraps a sql.DB with additional functionality for trace loading.
type DB struct {
	*sql.DB
	path string
...
// Create creates a new DuckDB at the given path and loads the trace into it.
func Create(duckPath string, traceData io.Reader) (*DB, error) {
	...
	stdlibMacro, err := stdlibMacro()
	if err != nil {
		return nil, errors.Join(err, db.Close())
	}
	if _, err := db.Exec(stdlibMacro); err != nil {
		return nil, errors.Join(err, db.Close())
	}

	if err := db.loadTrace(context.Background(), traceData); err != nil {
		return nil, errors.Join(err, db.Close())
	}
	return db, nil
}
...
// Stack converts a trace.Stack to a stack_id in the database.
// The new API uses Stack.Frames() which returns an iter.Seq[StackFrame].
func (l *loader) Stack(s trace.Stack) (stackID uint64, err error) {
	...
	var frames []*frameRow
	// Use the iterator-based API: s.Frames() returns iter.Seq[StackFrame]
	for f := range s.Frames() {
		...
		if !ok {
			...
			if err = l.Append(
				...
				frame.Line,
				nil, // inlined - not available from trace.StackFrame
			); err != nil {
				return 0, err
			}
		}
